%--------------------------------------------------------------------------
% File that creates the Transiting HSCs with MPP Self-Renewal model. 
% It stores it in a mat-file in the 'model' file.
%--------------------------------------------------------------------------
clear all;

syms k1 k2 p1 p2 p3 p4 p5 p6 p7 p8 k3 d4 d5 d6 d7 d8 d9 r12 r21 ...
...
% parameters:
p = [k1; k2; p1; p2; p3; p4; p5; p6; p7; p8; ...
        k3; d4; d5; d6; d7; d8; d9; r12; r21];  
    
% dynamic equations:    
f = [(k1-p1-r12)*HSC1 + r21*HSC2;
     (k2-p2-r21)*HSC2 + r12*HSC1;
     p1*HSC1 + p2*HSC2 - (p3+p6-k3)*MPP;
     p3*MPP - (d4+p4+p5)*CMP;
     p4*CMP - d5*GMP;
     p5*CMP - (d6+p7)*MEP;
     p8*EPCD71p - d9*EPCD71n;
     p7*MEP - (d8+p8)*EPCD71p;
     p6*MPP - d7*MLP;]; 
...
save('../models/TransitingHSC_MPPSelfRenewal.mat','x','h','u','p','f','ics','known_ics');