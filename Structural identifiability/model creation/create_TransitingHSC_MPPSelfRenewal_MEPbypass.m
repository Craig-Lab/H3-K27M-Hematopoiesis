%--------------------------------------------------------------------------
% File that creates the Transiting HSCs with MPP Self-Renwal & MEP Bypass model. 
% It stores it in a mat-file in the 'model' file.
%--------------------------------------------------------------------------
clear all;

syms k1 k2 p1 p2 p3 p4 p5 p6 p7 p8 p9 k3 d4 d5 d6 d7 d8 d9 r12 r21 ...
...
f = [(k1-p1-r12)*HSC1 + r21*HSC2;
     ...
     p1*HSC1 + p2*HSC2 - (p3+p6+p9-k3)*MPP;
     p3*MPP - (d4+p4+p5)*CMP;
     p4*CMP - d5*GMP;
     p5*CMP - (d6+p7)*MEP;
...
save('../models/TransitingHSC_MPPSelfRenewal_MEPBypass.mat','x','h','u','p','f','ics','known_ics');