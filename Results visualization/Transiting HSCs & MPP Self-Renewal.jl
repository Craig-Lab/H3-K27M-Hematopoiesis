...
"p<sub>MEP\u2192EP+</sub>", "p<sub>EP+\u2192EP-</sub>", "k<sub>MPP</sub>", "d<sub>CMP</sub>", "p<sub>GMP\u2192M</sub>", "d<sub>MEP</sub>", "p<sub>MLP\u2192M</sub>", "d<sub>EP+</sub>", "p<sub>EP-\u2192M</sub>", 
...
for i = 1:nPops
    dCell_WT[i] = permutedims(
        hcat(
            cellCount_WT[:,i][i28], cellCount_WT[:,i][i56], cellCount_WT[:,i][i84], cellCount_WT[:,i][i112]
            )
        );
    dCell_K27M[i] = permutedims(
    hcat(
        cellCount_K27M[:,i][i28], cellCount_K27M[:,i][i56], cellCount_K27M[:,i][i84], cellCount_K27M[:,i][i112]
        )
    );
end
...
#            [aHSC1  p1_MPP aHSC2  p2_MPP pM_CMP pC_GMP pC_MEP pM_MLP pME_EP pEP_EP kMPP   dCMP   dGMP   dMEP   dMLP   dEP+   dEP-   r12    r21   ];
L_WT   = vec([0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000]);
U_WT   = vec([0.0200 0.5000 0.0400 0.5000 8.0000 4.0000 4.0000 0.1000 10.000 3.0000 6.0000 2.0000 1.5000 2.0000 0.1000 2.0000 2.0000 0.0500 0.0200]);

L_K27M = vec([0.0000 0.0000 0.0000 0.0000 1.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000 0.0000]);
U_K27M = vec([0.0700 0.5000 0.0500 0.5000 8.0000 2.0000 2.5000 0.0700 7.0000 1.5000 6.0000 1.0000 2.0000 2.0000 0.2000 1.0000 2.0000 0.0500 0.0500]);
...
# Model where we consider HSC1 and HSC2 as transiting populations of LT-HSCs.
        ...
        k3  = param[11];
        d4  = param[12]; d5  = param[13];
        d6  = param[14]; d7  = param[15];
        d8  = param[16]; d9  = param[17];
        r12 = param[18]; r21 = param[19];
        ...
        dMPP  = p1*HSC1 + p2*HSC2 - (p3+p6-k3)*MPP;
        dCMP  = p3*MPP - (d4+p4+p5)*CMP;
        dGMP  = p4*CMP - d5*GMP;
        dMEP  = p5*CMP - (d6+p7)*MEP;
        dEPp  = p7*MEP - (d8+p8)*EPp;
        dEPn  = p8*EPp - d9*EPn;
        dMLP  = p6*MPP - d7*MLP;
...
function costfunction!(integrator, param::AbstractVector, data::CostData)
    ...
    for i = 1:nCellPop
        ...
        # Extract solution at four the timepoints
        if length(sol.u) < 4
            return Inf
        end
        m1 = sol.u[1][1, i]
        ...
        m3 = sol.u[3][1, i]
        m4 = sol.u[4][1, i]
        model_mat = hcat(m1, m2, m3, m4)

        NSE += sum(((data_mat .-  model_mat) ./ μ).^2)
    end

    return isfinite(NSE) ? NSE : Inf
end

function unstable_params(param)

    p3 = param[5]
    p6 = param[8]
    k3 = param[11]

    # MPP net growth becomes positive
    if p3 + p6 <= k3
        return true
    end
